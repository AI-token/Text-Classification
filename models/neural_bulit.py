...
from keras.layers.core import Dense, initializers, Flatten, Dropout
    ...
    for n in range(len(net_shape)):
        if net_shape[n]['name'] == 'InputLayer':
            ...
            if 'recurrent_dropout' not in net_shape[n]:
                net_shape[n].update({'activation': 0.})

            model.add(LSTM(units=net_shape[n]['units'],
                           activation=net_shape['activation'],
                           recurrent_activation=net_shape['recurrent_activation'],
                           implementation=1,
                           dropout=net_shape['dropout'],
                           recurrent_dropout=net_shape['recurrent_dropout'],
